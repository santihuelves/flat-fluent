-- =====================================================================
-- BLOQUE 20 - Compatibilidad base desde el perfil + bonus del test
-- =====================================================================
-- Antes: si no había ≥8 respuestas comunes en el test exhaustivo, la
-- API devolvía INSUFFICIENT_COMMON_ANSWERS y no se mostraba ningún %.
-- Ahora: siempre se calcula un score base con los datos del perfil
-- (lifestyle, intenciones, ubicación, idiomas, presupuesto, dealbreakers)
-- y, si además hay test exhaustivo, se mezcla 40% perfil + 60% test.

-- 1) Score 0..100 a partir SOLO del perfil --------------------------------
...
DECLARE
  ...
  ints_a jsonb;
  ints_b jsonb;
  -- per-bloque (0..1) + flag de "hay datos para puntuar"
...
BEGIN
  ...
  -- Recopilamos intenciones activas
  ...
  -- (1) LIFESTYLE / convivencia: traits clave + solape de lifestyle_tags
  IF cp_a IS NOT NULL AND cp_b IS NOT NULL THEN
    DECLARE
      ...
      sub_w float := 0;
    BEGIN
      -- Comparar traits gestionados (smoker, pet, household, minor)
      FOR v_a IN
      ...
      -- Solape de lifestyle_tags genéricos (Jaccard, ignorando trait_/auto_)
      DECLARE
  ...
  -- (2) INTENCIONES: pares complementarios
  ...
  IF bmin_a IS NOT NULL AND bmax_a IS NOT NULL AND bmin_b IS NOT NULL AND bmax_b IS NOT NULL THEN
    ...
    IF overlap_hi >= overlap_lo THEN
      -- ratio de solape sobre el rango más estrecho
  ...
  IF mins_a IS NOT NULL AND mins_b IS NOT NULL THEN
    budget_has := true;
    -- Mezclamos: 70% rango precio, 30% estancia
  ...
  -- (6) DEALBREAKERS: penalización si el dealbreaker de A se cumple en B (y viceversa)
  DECLARE
  ...
  BEGIN
    IF array_length(db_a, 1) IS NOT NULL OR array_length(db_b, 1) IS NOT NULL THEN
      ...
      FOREACH db IN ARRAY db_b LOOP
        ...
        END IF;
      END LOOP;
      dealbreak_pen := least(1.0, hits * 0.5); -- 0 conflictos = penalización 0, 2+ = 1.0
    END IF;
  END;

  -- Composición ponderada (sin contar bloques sin datos)
  ...
  -- Aplicar penalización dealbreakers como resta directa al final (hasta -40 pts)
...
-- 2) Reemplazo de compute_and_cache_guarded: perfil siempre + test opcional
...
DECLARE
  ...
  scoring_model text := 'convinter_profile_v1+test_v3';
...
BEGIN
  ...
  -- Devolver caché si sigue siendo modelo actual
  ...
  -- Cache obsoleto o ausente: limpiar y recalcular
  IF cache_hit THEN
  ...
  -- (B) Score de test exhaustivo (v3, igual que migración 16)
  WITH
...
-- Invalidar cachés viejos
DELETE FROM public.convinter_compat_cache
WHERE coalesce(breakdown->>'scoring_model','') <> 'convinter_profile_v1+test_v3';

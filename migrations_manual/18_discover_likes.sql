-- =====================================================================
-- BLOQUE 18 - Likes silenciosos en Discover + match mutuo automático
-- =====================================================================
-- Cuando dos usuarios pulsan el corazón en /discover el uno sobre el
-- otro, se crea automáticamente consent nivel 1 y se notifica a ambos.
-- Mientras el like es unilateral, el destinatario NO se entera.
...
BEGIN
  ...
  -- Match: subir consent a nivel 1 (sin bajar nivel ya existente)
  a := least(me, p_target);
  ...
  -- Notificar a ambos sólo si el consent es nuevo (antes era 0/NULL)
  IF coalesce(existing_consent, 0) < 1 THEN
    PERFORM public.convinter_notify(
      me, 'MATCH_CREATED',
      jsonb_build_object('with', p_target)
    );
    PERFORM public.convinter_notify(
      p_target, 'MATCH_CREATED',
      jsonb_build_object('with', me)
    );
  END IF;

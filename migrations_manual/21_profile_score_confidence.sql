-- =====================================================================
-- BLOQUE 21 - Calibración de confianza para profile-only
-- =====================================================================
-- Problema: si solo coincidían 2 señales débiles (idioma + ciudad), el
-- score podía dar 100% aunque el resto del perfil estuviera vacío y la
-- otra persona no hubiera hecho el test exhaustivo.
--
-- Solución:
--  1) Exigir cobertura mínima de 40 puntos de peso (sobre 90 posibles).
--  2) Exigir que al menos uno de los bloques fuertes (lifestyle o
--     intenciones) tenga datos. Sin ninguno de los dos, el resto es
--     decorativo.
--  3) Aplicar factor de confianza score * (0.6 + 0.4 * weight/90).
--  4) No contar dealbreakers en signals_used (es modificador, no señal).
--  5) Bump del scoring_model a v2 e invalidación del caché.
...
BEGIN
  ...
  -- Sin datos suficientes: cobertura mínima O ningún bloque fuerte
  ...
  -- Factor de confianza: 0.6 base + hasta 0.4 según cobertura
  confidence := 0.6 + 0.4 * least(1.0, weight_sum / max_weight);
  final_score := round(raw_score * confidence)::int;

  -- Penalización dealbreakers como resta directa (no cuenta como señal)
...
-- Wrapper: actualizar scoring_model y mensaje cuando no hay datos

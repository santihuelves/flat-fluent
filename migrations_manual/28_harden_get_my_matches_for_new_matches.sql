-- =====================================================================
-- BLOQUE 28 - Blindar convinter_get_my_matches frente a matches recientes
-- =====================================================================
-- Garantiza que Mensajes no falle cuando hay un MATCH_CREATED reciente
-- pero todavia no existe chat, ultimo mensaje ni cache de compatibilidad.
-- Defaults seguros: compatibility_score=0, compatibility_reasons=[],
-- chat_id=null, last_message=null, unread_count=0.
...
DECLARE
  ...
  compatibility_reasons_val jsonb;
  last_msg record;
  unread_count int;
  chat_id_val uuid;
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      BEGIN
        SELECT
          coalesce(score, 0)::int,
          coalesce(breakdown->'reasons', '[]'::jsonb)
        INTO compatibility_score_val, compatibility_reasons_val
      ...
      IF compatibility_score_val IS NULL THEN
        compatibility_score_val := 0;
      END IF;
      IF compatibility_reasons_val IS NULL THEN
        compatibility_reasons_val := '[]'::jsonb;
      END IF;
      ...
      IF chat_id_val IS NOT NULL THEN
        ...
        IF last_msg IS NOT NULL AND last_msg.created_at IS NOT NULL THEN
          last_message_json := jsonb_build_object(
            'body', last_msg.body,
      ...
      matches_array := matches_array || jsonb_build_object(
        ...
        'compatibility_reasons', compatibility_reasons_val,
    ...
    EXCEPTION WHEN OTHERS THEN
      -- Si un match concreto falla, lo saltamos sin romper la bandeja
      CONTINUE;
    END;
  END LOOP;

-- =====================================================================
-- BLOQUE 24 - Contar hábitos auto_% del perfil en la compatibilidad
-- =====================================================================
-- Antes: convinter_score_profile_pair excluía los tags `auto_%` del
-- bloque lifestyle, por lo que los hábitos visibles del perfil (sleep,
-- noise, cleanliness, social, etc., guardados como `auto_*`) no
-- aportaban señal a la compatibilidad orientativa.
-- Ahora: los `auto_%` se incluyen en el solape Jaccard de lifestyle.
-- Se sube scoring_model a `convinter_profile_v4+test_similarity_v5` y se
-- invalidan cachés antiguas.

-- 1) Recompute de convinter_score_profile_pair contando auto_%
...
BEGIN
  IF a_user IS NULL OR b_user IS NULL OR a_user = b_user THEN
    RETURN jsonb_build_object('score', 0, 'signals_used', 0);
  END IF;
  ...
  -- (1) LIFESTYLE / convivencia: traits clave + solape de lifestyle_tags (incluye auto_%)
  IF cp_a IS NOT NULL AND cp_b IS NOT NULL THEN
    ...
    BEGIN
      ...
      -- Solape de auto_<dimension>_<valor> por dimensión (similitud por dimensión)
      DECLARE
        auto_a text[] := ARRAY(SELECT t FROM unnest(tags_a) t WHERE t LIKE 'auto\_%' ESCAPE '\');
        auto_b text[] := ARRAY(SELECT t FROM unnest(tags_b) t WHERE t LIKE 'auto\_%' ESCAPE '\');
        dims_a text[];
        dims_b text[];
        dims_common text[];
        d text;
        ...
        total int := 0;
      BEGIN
        IF array_length(auto_a,1) IS NOT NULL AND array_length(auto_b,1) IS NOT NULL THEN
          -- extraer "auto_<dimension>" (parte antes del último '_')
          SELECT array_agg(DISTINCT substring(t from '^(auto_[^_]+(?:_[^_]+)*)_[^_]+$'))
            INTO dims_a FROM unnest(auto_a) t;
          SELECT array_agg(DISTINCT substring(t from '^(auto_[^_]+(?:_[^_]+)*)_[^_]+$'))
            INTO dims_b FROM unnest(auto_b) t;
          SELECT array_agg(d) INTO dims_common
            FROM (SELECT unnest(dims_a) INTERSECT SELECT unnest(dims_b)) z(d);
          IF dims_common IS NOT NULL AND array_length(dims_common,1) > 0 THEN
            FOREACH d IN ARRAY dims_common LOOP
              total := total + 1;
              IF EXISTS (
                SELECT 1 FROM unnest(auto_a) ta
                JOIN unnest(auto_b) tb ON ta = tb
                WHERE ta LIKE d || '\_%' ESCAPE '\'
              ) THEN
                matched := matched + 1;
              END IF;
            END LOOP;
            IF total > 0 THEN
              sub_w := sub_w + 2; -- doble peso por ser señales reales del perfil
              sub_score := sub_score + 2 * (matched::float / total::float);
            END IF;
          END IF;
        END IF;
      END;

      -- Solape de lifestyle_tags genéricos no-trait y no-auto (Jaccard)
      DECLARE
  ...
  final_score := round(weighted_sum / weight_sum * 100)::int;

  IF dealbreak_has THEN
...
-- 2) Bump scoring_model en convinter_compute_and_cache_guarded
...
DECLARE
  ...
  scoring_model text := 'convinter_profile_v4+test_similarity_v5';
...
-- 3) Invalidar cachés con modelo distinto al nuevo
DELETE FROM public.convinter_compat_cache
WHERE coalesce(breakdown->>'scoring_model','') <> 'convinter_profile_v4+test_similarity_v5';

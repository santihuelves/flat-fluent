...
DECLARE
  ...
  cached_model text := NULL;
  score_num integer := 0;
  common_n integer := 0;
  min_common_questions integer := 8;
  mismatches jsonb := '[]'::jsonb;
  scoring_model_tag constant text := 'convinter_full_similarity_v2';
BEGIN
  IF me IS NULL THEN
  ...
  IF cache_hit THEN
    cached_common_n := CASE
      ...
      ELSE 0
    END;
    cached_model := cached.breakdown->>'scoring_model';
  END IF;
  ...
  IF cache_hit
     ...
     AND cached_model = scoring_model_tag THEN
    RETURN jsonb_build_object(
      'ok', true,
      'cached', true,
      'can_show_score', true,
  ...
  IF cache_hit AND (cached_common_n < min_common_questions OR cached_model IS DISTINCT FROM scoring_model_tag) THEN
  ...
  VALUES (
    a, b, lvl, score_num,
    jsonb_build_object(
      'scoring_model', scoring_model_tag,
      'common_questions', common_n,
...
DELETE FROM public.convinter_compat_cache
WHERE COALESCE(breakdown->>'scoring_model', '') <> 'convinter_full_similarity_v2'
   OR COALESCE(
        CASE

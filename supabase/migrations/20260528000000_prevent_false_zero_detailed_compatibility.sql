-- Prevent false 0% detailed compatibility when users do not share enough test answers.
-- Scenario answers are scored with a small similarity matrix so compatible styles are not treated as total mismatches.
...
BEGIN
  ...
  IF cache_hit AND cached_common_n >= min_common_questions THEN
    RETURN jsonb_build_object(
      'ok', true,
      'cached', true,
      'can_show_score', true,

-- Blindar convinter_get_my_matches frente a matches sin chat/cache
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      BEGIN
        SELECT coalesce(score, 0)::int, coalesce(breakdown->'reasons', '[]'::jsonb)
      ...
      IF compatibility_score_val IS NULL THEN compatibility_score_val := 0; END IF;
      IF compatibility_reasons_val IS NULL THEN compatibility_reasons_val := '[]'::jsonb; END IF;
      ...
      IF chat_id_val IS NOT NULL THEN
        ...
        IF unread_count IS NULL THEN unread_count := 0; END IF;
      END IF;

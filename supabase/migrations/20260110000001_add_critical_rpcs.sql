-- Add critical RPCs for test status and matches functionality

-- 1. Get test status for a user (needed for Profile/Discover UI)
...
BEGIN
  ...
  -- If checking another user, verify not blocked
...
-- 2. Get matches for current user (users with mutual consent and compatibility)
...
BEGIN
  ...
  -- Get users with mutual consent (level >= 1)
  FOR match_record IN
    SELECT 
      CASE 
  ...
  LOOP
    ...
    -- Get compatibility score (if exists in cache)
    SELECT score, breakdown
    INTO compatibility_data
    FROM public.convinter_compat_cache
    WHERE (user_a = me AND user_b = match_record.other_user)
       OR (user_a = match_record.other_user AND user_b = me)
    ...
    WHERE (c.user_a = me AND c.user_b = match_record.other_user)
       OR (c.user_a = match_record.other_user AND c.user_b = me);
    ...
    IF chat_id_val IS NOT NULL THEN
      ...
      -- Count unread messages (simple: messages from other user after my last read)
      -- Note: This is simplified - proper implementation would track read receipts
      SELECT COUNT(*)
      INTO unread_count
      FROM public.convinter_messages m
      WHERE m.chat_id = chat_id_val
        AND m.sender_id = match_record.other_user
        AND m.created_at > COALESCE(
          (SELECT MAX(created_at) FROM public.convinter_messages WHERE chat_id = chat_id_val AND sender_id = me),
...
-- 3. Enable Realtime for messages table (critical for chat)
-- Note: This needs to be run in Supabase SQL Editor AND Realtime must be enabled in Dashboard
ALTER PUBLICATION supabase_realtime ADD TABLE public.convinter_messages;

-- 4. Enable Realtime for notifications table (high priority)
ALTER PUBLICATION supabase_realtime ADD TABLE public.convinter_notifications;

-- 5. Helper function to mark chat messages as read (optional but useful)
...
BEGIN
  ...
  -- In future, could update a last_read_at column on convinter_chats or separate read_receipts table

-- ===== BLOQUE 20 - Compatibilidad base perfil + bonus test =====
...
BEGIN
  ...
  IF cp_a IS NOT NULL AND cp_b IS NOT NULL THEN
    DECLARE
      ...
      sub_w float := 0;
    BEGIN
      FOR v_a IN SELECT unnest(ARRAY['trait_smoker_','trait_pet_','trait_household_','trait_minor_']) LOOP
        DECLARE
...
BEGIN
  ...
  IF consent_lvl < lvl THEN
    RETURN jsonb_build_object('ok', false,
      ...
      'consent_level', consent_lvl, 'required_level', lvl);
  END IF;
  ...
  IF cache_hit AND cached.breakdown->>'scoring_model' = scoring_model THEN
    ...
    RETURN jsonb_build_object('ok', true, 'cached', true, 'can_show_score', true,
      'score', cached.score, 'detail_level', lvl, 'common_questions', cached_common_n,
  ...
  ELSIF profile_score IS NOT NULL THEN
    ...
    test_score := NULL;
  ELSE
    RETURN jsonb_build_object('ok', false, 'code', 'INSUFFICIENT_PROFILE_DATA',
      'can_show_score', false, 'common_questions', common_n,
      'profile_signals_used', profile_signals,
      'message', 'Aun no hay datos suficientes en los perfiles para calcular la compatibilidad.');
  END IF;
  ...
  VALUES (a, b, lvl, final_score,
    jsonb_build_object('scoring_model', scoring_model, 'source', source_str,
      'profile_score', profile_score, 'profile_signals_used', profile_signals,
      'profile_parts', prof_jsonb->'parts', 'test_score', test_score,
      'common_questions', common_n, 'test_available', test_available,
      'mismatches', mismatches), now())
  ...
  RETURN jsonb_build_object('ok', true, 'cached', false, 'can_show_score', true,
    'score', final_score, 'detail_level', lvl, 'common_questions', common_n,
    'breakdown', jsonb_build_object('scoring_model', scoring_model, 'source', source_str,
      'profile_score', profile_score, 'profile_signals_used', profile_signals,
      'profile_parts', prof_jsonb->'parts', 'test_score', test_score,
      'common_questions', common_n, 'test_available', test_available,
